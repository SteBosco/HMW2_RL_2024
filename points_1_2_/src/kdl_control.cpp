...
//    return M * (Jpinv*y + (I-Jpinv*J)*(/*- 10*grad*/ - 1*robot_->getJntVelocities())) + robot_->getCoriolis();
// }
    ...
    double _Kdp, double _Kdo)
{
    ...
    Kd.block(3, 3, 3, 3) = _Kdo * Eigen::Matrix3d::Identity();
       ...
       // Eigen::Matrix<double, 6, 7> J = toEigen(JEE);
        ...
        // Torques
        return M * (Jpinv * y + (I - Jpinv * JEE.data) * (-1 * robot_->getJntVelocities())) + robot_->getCoriolis();
